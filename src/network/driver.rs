...
pub enum NetEvent<'a> {
    ...
    /// This event is only generated after a [`crate::network::NetworkController::connect_async()`]
    /// call.
    /// The event contains the endpoint of the connection attempt
    /// (same endpoint returned by the `connect_async()` method).
    /// The `bool` indicates if the connection could be established or not.
    ///
    /// Note that this event will only be generated by connection-oriented transports as *TCP*.
    Connected(Endpoint, bool),

    /// New endpoint has been accepted by a listener.
...
impl std::fmt::Debug for NetEvent<'_> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let string = match self {
            Self::Connected(endpoint, ok) => format!("Connected({}, {})", endpoint, ok),
...
impl<R: Remote, L: Local<Remote = R>> Driver<R, L> {
    fn process_remote_write(&self, id: ResourceId, mut event_callback: impl FnMut(NetEvent<'_>)) {
        if let Some(remote) = self.remote_registry.get(id) {
            ...
            event_callback(NetEvent::Connected(endpoint, true)); //TODO
