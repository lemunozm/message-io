...
impl From<u8> for Transport {
    fn from(adapter_id: u8) -> Transport {
        Transport::try_from(adapter_id).expect("Unknown adapter id.")
    }
}
...
impl<'a> Network {
    ...
    pub fn connect_tcp<A: ToSocketAddrs>(&mut self, addr: A) -> io::Result<Endpoint> {
        ...
        self.tcp_adapter.add_remote(addr)
    }
    ...
    pub fn connect_udp<A: ToSocketAddrs>(&mut self, addr: A) -> io::Result<Endpoint> {
        let addr = addr.to_socket_addrs().unwrap().next().unwrap();
        self.udp_adapter.add_remote(addr)
    }
    ...
    -> io::Result<(ResourceId, SocketAddr)> {
        let addr = addr.to_socket_addrs().unwrap().next().unwrap();
        self.tcp_adapter.add_listener(addr)
    }
    ...
    -> io::Result<(ResourceId, SocketAddr)> {
        let addr = addr.to_socket_addrs().unwrap().next().unwrap();
        self.udp_adapter.add_listener(addr)
    }
    ...
    pub fn remove_resource(&mut self, resource_id: ResourceId) -> Option<()> {
        match resource_id.adapter_id().into() {
    ...
    where OutMessage: Serialize {
        ...
        match endpoint.resource_id().adapter_id().into() {
